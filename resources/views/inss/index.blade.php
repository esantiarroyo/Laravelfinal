...
<body class="bg-primary">
    ...
    <div class="bg-primary-subtle mt-3 mb-3 pt-3 pb-3 d-flex justify-content-center">
                    ...
                    <table class="table table-bordered border-primary">
                        <thead>
                          ...
                          <tbody>
                            <tr>
                                <td>
                                    <h5 href="{{ route('inss.show', $ins) }}">
                                    {{ $ins->name }}
                                    </h5>
                                </td>

                                <td>
                                    <button type="button" class="btn btn-light"><a href="{{ route('inss.edit', $ins) }}">Edit</a></button>
                                </td>

                                <td>
                                    <form action="{{ route('inss.destroy', $ins) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
        ...
        <div>
            <a href="{{ route('inss.create') }}">Crear nuevo Instructor</a>
        </div>
    </div>
